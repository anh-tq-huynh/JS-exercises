...
for (let i of picArray) {
  //Create the article
  let location = document.getElementById('pictures')
  let addArticle = location.appendChild(document.createElement('article'))

  //Add heading to the article
  let addHeading = addArticle.appendChild(document.createElement('h2'))
  addHeading.innerHTML = i.title
  //Add figure to the article
  let addFigure = addArticle.appendChild(document.createElement('figure'))
  //Add img & figcaption
  let addImg =addFigure.appendChild(document.createElement('img'))
  addImg.src = i.image.medium
  addImg.alt = i.title
  let addCap = addFigure.appendChild(document.createElement('figcaption'))
  addCap.innerHTML = i.caption
  //Add description
  let addDes = addArticle.appendChild(document.createElement('p'))
  addDes.innerHTML = i.description


}
let dialog = document.querySelector("dialog")
let selectArticle = document.querySelectorAll('article')
selectArticle.forEach(article => {
  article.addEventListener('click', e => {
    let title = e.currentTarget.getElementsByTagName('h2')[0].textContent
    console.log(title)
    let index = picArray.findIndex(x => x.title === title)
    console.log(index)
    dialog.showModal()
    Modal(index)
...
let closeBtn = document.querySelector('dialog span')
closeBtn.addEventListener('click', e => {
  dialog.close()
})


function Modal(articleNo) {
  let location = document.querySelector("dialog").querySelector("img")
  location.src = picArray[articleNo].image.large
  location.alt = picArray[articleNo].caption
